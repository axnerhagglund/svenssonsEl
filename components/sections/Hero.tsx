    ...
    <>
      ...
      >
        ...
        <div className="w-full max-w-content mx-auto px-5 sm:px-8 flex-1 flex items-center min-h-0">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 py-8 lg:py-14 w-full items-center">

            {/* Text side */}
            <div className="text-center lg:text-left max-w-2xl mx-auto lg:max-w-none lg:mx-0 w-full">
              <h1
                className="anim-1 font-head text-[clamp(2.8rem,6vw,4.5rem)] font-black text-white leading-[0.95] tracking-tight mb-6 text-balance"
                style={{ opacity: 0 }}
              >
                {headlinePrefix}{' '}
              ...
              <p
                className="anim-2 text-[17px] text-white/65 leading-relaxed mb-9 max-w-[500px] mx-auto lg:mx-0"
                style={{ opacity: 0 }}
              >
                {subtext}
              </p>
              ...
              <div className="anim-4 flex justify-center lg:justify-start" style={{ opacity: 0 }}>
                <a
                  href={phoneHref}
                  className="inline-flex items-center gap-2.5 text-sm text-white/60 hover:text-white/80
            ...
            <div className="relative hidden lg:block">
              <div
                className="relative rounded-2xl overflow-hidden aspect-[4/5] max-h-[620px] ml-auto
        ...
        {/* Stats bar — pinned at the bottom, always visible within 100dvh */}
        <div className="relative z-10 border-t border-white/[0.08] bg-white/[0.03] backdrop-blur-sm shrink-0">
          <div className="w-full max-w-content mx-auto px-5 sm:px-8">
            <div className="grid grid-cols-3 divide-x divide-white/[0.08]">
              {stats.map((stat, i) => (
                <div
                  key={stat.label}
                  className={`stat-${i + 1} flex items-center gap-3 sm:gap-5 px-4 sm:px-8 py-6`}
                  style={{ opacity: 0 }}
                >
                  <span className="font-head text-[2.25rem] sm:text-[3rem] font-black text-white leading-none tabular-nums">
                    {stat.value}
                  </span>
                  <span className="text-[11px] sm:text-[13px] leading-snug uppercase tracking-wider" style={{ color: 'rgba(201,148,26,0.75)' }}>
                    {stat.label}
                  </span>
                </div>
              ))}
